...
class Line(object):
    ...
    def __eq__(self, line2):
        ...
        if self.normal_vector.is_zero_vector():
            if not line2.normal_vector.is_zero_vector():
                return False
            else:
                diff = self.constant_term - line2.constant_term
                return MyDecimal(diff).is_near_zero()
        elif line2.normal_vector.is_zero_vector():
            return False

        if self.is_parallel_with(line2):
            diff = self.basepoint - line2.basepoint
    ...
    def find_intersection(self, line2):
        ...
        ad_minus_bc = a*d-b*c
        if MyDecimal(ad_minus_bc).is_near_zero():
            if self == line2:
                return self
            else:
                return None

        x = (d*k1 - b*k2) / ad_minus_bc
        y = (-c*k1 + a*k2) / ad_minus_bc
        return Vector([x, y])
